...
def main():
    print("=" * 60)
    print("ASSEMBLING MASTER UNIFIED DATASET WITH KITTI CLEAN ROADS")
    print("=" * 60)

    # Clean old test repo if present
    old_test = BASE / "05_CLEAN_PERFECT_ROADS"
    if old_test.exists():
        shutil.rmtree(old_test, ignore_errors=True)
    ...
    # 2. Chitholian Potholes (665 images)
    ...
    # 5. KITTI Clean Perfect Roads (500 pristine clean highway & urban road images)
    kitti_dir = BASE / "05_KITTI_CLEAN_PERFECT_ROADS" / "data_road"
    # Select camera RGB images from testing and training image_2
    kitti_imgs = list((kitti_dir / "testing" / "image_2").glob("*.png")) + list((kitti_dir / "training" / "image_2").glob("*.png"))
    random.seed(42)
    random.shuffle(kitti_imgs)
    selected_kitti = kitti_imgs[:600] # 600 pristine clean road frames
    for img_p in selected_kitti:
        all_pairs.append((img_p, [], f"kitti_clean_{img_p.stem}"))
    print(f"Loaded {len(selected_kitti)} pristine clean road negative pairs from KITTI.")
